"""
Slab NEB (VASP + ASE NEB), optimizer LBFGS. Five images: endpoints from
00/final_with_calculator.json and 04/final_with_calculator.json; internal 01–03.
For NEB, ASE suggests FIRE/MDMin/BFGS over BFGSLineSearch; LBFGS may behave
similarly to other quasi-Newton methods for CI-NEB. See https://ase-lib.org/ase/neb.html
"""
import os
import time
import subprocess
...
#   mpirun -np 3 python neb_LBFGS.py
...
if NEB_PARALLEL:
    if _mpi_world.size != N_INTERNAL:
        raise SystemExit(
            ...
            f'... python neb_LBFGS.py (got {_mpi_world.size} ranks).'
        )
...
opt = LBFGS(neb, trajectory=_traj, logfile=_log)
opt.run(fmax=FMAX)
