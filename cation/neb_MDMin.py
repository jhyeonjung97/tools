"""
Slab NEB (VASP + ASE NEB), optimizer MDMin. Five images: endpoints from
...
#   mpirun -np 3 python neb_MDMin.py
...
if NEB_PARALLEL:
    if _mpi_world.size != N_INTERNAL:
        raise SystemExit(
            ...
            f'... python neb_MDMin.py (got {_mpi_world.size} ranks).'
        )
...
opt = MDMin(neb, trajectory=_traj, logfile=_log)
opt.run(fmax=FMAX)
