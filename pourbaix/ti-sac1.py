...
concentration_aq = 1e-6
concentration_gas = 1e-12
...
parser.add_argument('--suffix', type=str, default='TiSAC1', help='Suffix for output filename')
...
surfs = [
    ...
    [-310.30495021, 1, 1, +0, 0, 1, 2, 0, 0, 0, 0, '*NO₂+*OH(adj)'], ##
    ...
    [-306.42159787, 1, 1, +0, 3, 0, 1, 0, 0, 0, 0, '*NH₃+*O(adj)'],
...
for pH in pHrange:
    ...
    for U in Urange:
        ...
        for k in range(nsurfs):
            if '(aq)' in surfs[k][11]:
                value = dg(k, pH, U, concentration=concentration_aq, n_ref=n_ref)
            elif '(g)' in surfs[k][11]:
                value = dg(k, pH, U, concentration=concentration_gas, n_ref=n_ref)
            else:
                value = dg(k, pH, U, concentration=1, n_ref=n_ref)
            values.append(value)
...
for Uindex, U in enumerate(Urange):
    ...
    for k in range(nsurfs):
        if '(aq)' in surfs[k][11]:
            value = dg(k, pH=target_pH, U=U, concentration=concentration_aq, n_ref=n_ref)
        elif '(g)' in surfs[k][11]:
            value = dg(k, pH=target_pH, U=U, concentration=concentration_gas, n_ref=n_ref)
        else:
            value = dg(k, pH=target_pH, U=U, concentration=1, n_ref=n_ref)
        values.append(value)
    sorted_values = sorted(range(len(values)), key=lambda k: values[k])
...
for k in range(2, len(surfs)):
    ...
    for i, U in enumerate(Urange):
        if '(aq)' in surfs[k][11]:
            energy = dg(k, target_pH, U, concentration=concentration_aq, n_ref=n_ref)
        elif '(g)' in surfs[k][11]:
            energy = dg(k, target_pH, U, concentration=concentration_gas, n_ref=n_ref)
        else:
            energy = dg(k, target_pH, U, concentration=1, n_ref=n_ref)
        energies[i] = energy
