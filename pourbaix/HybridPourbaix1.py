...
def find_ref_surface(surfs, unique_elements, remaining_elements, args):
    ...
    if args.show_ref:
        comp = ", ".join(f"{elem}: {ref_surf[elem]}" for elem in remaining_elements if elem in ref_surf)
        print(f"\nReference surface (index {ref_surf_idx}): {ref_surf['name']}, E_DFT: {ref_energy:.2f} eV")
        print(f"Composition: {comp}")
    return ref_surf_idx, ref_surf


def apply_formation_corrections(surfs, ref_surf, file_oh_counts, args):
    """Apply formation energy and optional Gibbs corrections."""
    ref_energy = ref_surf['E_DFT']

    for row in surfs:
        oh_count = file_oh_counts.get(row.get('_json', ''), 0)

        formation = (
            -(row['H'] - oh_count) * (gh - dgh)
            - (row['O'] - oh_count) * (go - dgo)
            - oh_count * (goh - dgoh)
        )
        gibbs = row['gibbs_corr'] if args.gibbs else 0.0
        row['E_DFT'] = row['E_DFT'] - ref_energy + formation + gibbs
...
def load_thermo_species(unique_elements, remaining_elements, args):
    """Load ions, solids, gases, liquids from thermodynamic data files."""
    script_dir = os.path.dirname(__file__)
    thermo_path = args.thermo_data or os.path.join(script_dir, 'thermodynamic_data.jsonc')
    ref_path = args.ref_energies or os.path.join(script_dir, 'reference_energies.json')

    thermo_data = load_jsonc(thermo_path) if os.path.exists(thermo_path) else {}
    ref_energies = {}
    ...
    for el in unique_elements:
        if el not in ref_energies:
            print(f"WARNING: Element '{el}' not found in reference_energies.json! "
...
def main(args, png_name, suffix):
    ...
    apply_formation_corrections(surfs, ref_surf, file_oh_counts, args)

    ions, solids, gases, liquids = [], [], [], []
    for el, el_ions, el_solids, el_gases, el_liquids in load_thermo_species(
        unique_elements, remaining_elements, args,
    ):
        ions.extend(el_ions)
