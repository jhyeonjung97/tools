...
def MAT_m_VEC(m, v):
    ...
    for i in range(len(m)):
        ...
        assert len(v) == len(
            m[i]
        ), 'Length of the matrix row is not equal to the length of the vector'
...
def parse_env_params(params):
    ...
    if len(tmp) != 4:
        print(
            "[parse_env_params]: ERROR there should be exactly four parameters"
        )
        sys.exit(1)
    #
    [first, last, nderiv,
...
def parse_freqdat(freqdat_fh, nat):
    ...
    #
    # all frequencies should be supplied, regardless of requested to calculate
...
def parse_modesdat(modesdat_fh, nat):
    ...
    for i in range(nat * 3):
        ...
        eigvecs[i] = eigvec
        norms[i] = sqrt(sum([abs(x)**2 for sublist in eigvec
                             for x in sublist]))
    #
    ...
    while True:
        ...
        if "Eigenvectors and eigenvalues of the dynamical matrix" in line:
            ...
            for i in range(nfreq):
                if i > 0:
                    outcar_fh.readline()  # empty line between modes
                line = outcar_fh.readline()
                # VASP format: "   1 f  =   20.26 THz  127.31 2PiTHz  675.87 cm-1  83.80 meV"
                p = re.search(r'(\d+\.?\d*(?:[eE][+-]?\d+)?)\s+cm-1', line)
                if p is None:
                    raise ValueError(
                        "[get_modes_from_OUTCAR]: Could not parse frequency line. "
                        "Expected '... XXX.XX cm-1 ...' in line. "
                        f"Got: {line!r}"
                    )
                eigvals.append(float(p.group(1)))
                #
                # X         Y         Z           dx          dy          dz
                outcar_fh.readline()
                #
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(
            "[__main__]: ERROR Couldn't open input file POSCAR.phon, exiting...\n"
        )
        sys.exit(1)
    #
    ...
    if os.path.isfile('freq.dat') and os.path.isfile('modes_sqrt_amu.dat'):
        ...
        try:
            ...
            print(
                "[__main__]: ERROR Couldn't open modes_sqrt_amu.dat, exiting...\n"
            )
            sys.exit(1)
        #
    ...
    #
        ...
        print(
            "[__main__]: Neither OUTCAR.phon nor freq.dat/modes_sqrt_amu.dat were found, nothing to do, exiting..."
        )
        sys.exit(1)
    #
        ...
        for j in range(len(disps)):
            ...
            try:
                ...
                if not args['use_poscar']:
                    print("[__main__]: File " + disp_filename +
                          " not found, preparing displaced POSCAR")
                ...
                if args['gen']:  # only generate POSCARs
                    ...
                    try:
                        ...
                        print(
                            f"[__main__]: {err} ERROR Couldn't find OUTCAR file, exiting..."
                        )
                        sys.exit(1)
                    #
                    outcar_fh = open(disp_filename, 'r')
            ...
            try:
                ...
                print(str(err))
                print("[__main__]: Moving " + disp_filename +
                      " back to 'OUTCAR' and exiting...")
                move(disp_filename, 'OUTCAR')
                sys.exit(1)
            #
            for m in range(3):
                for n in range(3):
                    ra[m][n] += eps[m][n] * coeffs[
                        j] / step_size * norm * vol / (4.0 * pi)
            #units: A^2/amu^1/2 =         dimless   * 1/A         * 1/amu^1/2  * A^3
        #
        alpha = (ra[0][0] + ra[1][1] + ra[2][2]) / 3.0
        beta2 = ((ra[0][0] - ra[1][1])**2 + (ra[0][0] - ra[2][2])**2 +
                 (ra[1][1] - ra[2][2])**2 + 6.0 *
                 (ra[0][1]**2 + ra[0][2]**2 + ra[1][2]**2)) / 2.0
        print()
        print(
            "! %4i  freq: %10.5f  alpha: %10.7f  beta2: %10.7f  activity: %10.7f "
            % (i + 1, eigval, alpha, beta2, 45.0 * alpha**2 + 7.0 * beta2))
        output_fh.write(
            "%03i  %10.5f  %10.7f  %10.7f  %10.7f\n" %
            (i + 1, eigval, alpha, beta2, 45.0 * alpha**2 + 7.0 * beta2))
        output_fh.flush()
    #
    output_fh.close()
