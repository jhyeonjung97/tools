...
def plot_mae_matrix():
    ...
    # 히트맵 생성 및 mappable 객체 저장
    ax = sns.heatmap(mae_matrix, 
                    annot=True, 
                    fmt='.3f',
                    cmap='YlOrRd',
    ...
    # 레이블 설정
    plt.xlabel('ionN normalization')
    plt.ylabel('ICOHP normalization')
    plt.title('Test MAE Matrix for Different Normalization Methods')
    ...
    plt.xticks(np.arange(4) + 0.5, [norm_labels[m] for m in norm_methods])
    plt.yticks(np.arange(4) + 0.5, [norm_labels[m] for m in norm_methods], rotation=0)
