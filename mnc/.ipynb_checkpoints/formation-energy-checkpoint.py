                ...
                for i, dz in enumerate(dzs):
                    for path in matching_paths:
                        ...
                        if os.path.exists(atoms_path):
                            ...
                            formation_energy = (energy 
                                                - metal_df.at[metal, 'energy']
                                                + standard_electrode_potentials[metal] 
                                                - C26N4_energy)
...
def plotting(df, df_relaxed, dzs, spins, ylabel, png_filename, ymin=None, ymax=None, yticks=None, color=None):
    ...
    if len(min_columns) == 0:
        ...
        return
    stable_dz = []
    for i, column in enumerate(min_columns):
        stable_dz.append(dzs[i])
    stable_dz = np.array(stable_dz)
    x_new = np.linspace(min(stable_dz), max(stable_dz), 300)
    if 'eV' in ylabel:
        start_idx = 0
