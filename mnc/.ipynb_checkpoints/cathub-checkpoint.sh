...
for dir in ${mnc}/0_clean/*d/*_*/most_stable; do
    ...
    dest_dir="${cathub}/${metal}N4C26-relaxed/001-${site1}/H2Ogas_-0.5H2gas_star__OHstar"
    mkdir -p "${dest_dir}"
    cp "${dir}/oh/final_with_calculator.json" "${dest_dir}/OH.json"
    # echo "Copied oh/final_with_calculator.json to ${dest_dir}"
    
    dest_dir="${cathub}/${metal}N4C26-relaxed/001-${site1}/2.0H2Ogas_-1.5H2gas_star__OOHstar"
    mkdir -p "${dest_dir}"
    ...
    for i in {0..6}; do
        dz=${dzs[$i]}
        dest_dir="${cathub}/${metal}N4C26-${dz}/001-${site1}"
        mkdir -p "${dest_dir}"
...
for dir in ${mnc}/1_O/*_*/most_stable; do
    ...
    for i in {0..6}; do
        dz=${dzs[$i]}
        dest_dir="${cathub}/${metal}N4C26-${dz}/001-${site1}/H2Ogas_-1.0H2gas_star__Ostar"
        mkdir -p "${dest_dir}"
...
for dir in ${mnc}/2_OH/*_*/most_stable; do
    ...
    for i in {0..6}; do
        dz=${dzs[$i]}
        dest_dir="${cathub}/${metal}N4C26-${dz}/001-${site1}/H2Ogas_-0.5H2gas_star__OHstar"
        mkdir -p "${dest_dir}"
...
for dir in ${mnc}/3_OOH/*_*/most_stable; do
    ...
    for i in {0..6}; do
        dz=${dzs[$i]}
        dest_dir="${cathub}/${metal}N4C26-${dz}/001-${site1}/2.0H2Ogas_-1.5H2gas_star__OOHstar"
        mkdir -p "${dest_dir}"
...
for dir in ${mnc}/pourbaix/*_*/*/most_stable; do
    ...
    if [[ -n "${rxn}" ]] && [[ -f "${dir}/final_with_calculator.json" ]]; then
        dest_dir="${cathub}/${metal}N4C26-relaxed/001-${site}/${rxn}"
        mkdir -p "${dest_dir}"
        cp "${dir}/final_with_calculator.json" "${dest_dir}"
        # echo "Copied final_with_calculator.json to ${dest_dir}"
        if [[ ! ${site} == ${site1} ]]; then
            cp "${cathub}/${metal}N4C26-relaxed/001-${site1}/empty_slab.json" "${cathub}/${metal}N4C26-relaxed/001-${site}"
        fi
    fi
done
