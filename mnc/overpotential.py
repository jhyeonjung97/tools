...
def main():
    for m, metal in enumerate(metals):
        ...
        gibbs_energies.to_csv(f'/pscratch/sd/j/jiuy97/6_MNC/figures/overpotential/{row}_{group}{metal}_gibbs.tsv', sep='\t', float_format='%.2f')
        spin_cross_over.to_csv(f'/pscratch/sd/j/jiuy97/6_MNC/figures/overpotential/{row}_{group}{metal}_spin.tsv', sep='\t')
    ...
    scaling_relationship.to_csv('/pscratch/sd/j/jiuy97/6_MNC/figures/overpotential/scaling_relationship.tsv', sep='\t', float_format='%.2f')
...
def scaling(x, y, ads1, ads2, scaling_relationship, metals, xmin, xmax, ymin, ymax, txtx, txty):
    ...
    plt.ylim(ymin, ymax)
    plt.tight_layout()
    plt.savefig(f'/pscratch/sd/j/jiuy97/6_MNC/figures/overpotential/scaling_relationship_{ads1}_{ads2}.png')
...
def plotting(gibbs_energies, spin_cross_over, row, group, metal, rxn, rds, overpotential, ymin, ymax):
    if gibbs_energies.isna().all().all():
        ...
        return
    png_filename = f'/pscratch/sd/j/jiuy97/6_MNC/figures/overpotential/{row}_{group}{metal}_{rxn}.png'
    marker_size = 0.03
