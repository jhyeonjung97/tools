...
df['energy'] = df['G'] - df.loc['E0', 'G'] # - water * df['#O']
print(df)
...
def plot_pourbaix(entries, png_name):
    ...
    if 'bulk' in png_name:
        ax.text(7, 0.6, r"$\mathbf{E_{0}}$", fontsize=14, ha='center')
        ax.text(7, -0.75, r"$\mathbf{E_{0}\text{*}H}$", fontsize=14, ha='center')
        ax.text(7, -1.6, r"$\mathbf{E_{0}\text{*}H_{2}}$", fontsize=14, ha='center')
