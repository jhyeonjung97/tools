...
df['energy'] = df['G'] - df.loc['E3', 'G'] # - water * df['#O']
print(df)
...
def plot_pourbaix(entries, png_name):
    ...
    ax.text(7, -0.5, r'E°$_{N_{2}/NH_{3}}$ = 0.05 V vs RHE', color='blue', rotation=-14, fontsize=14, ha='center')
    ...
    if 'bulk' in png_name:
        ax.text(7, 0.5, r"$\mathbf{E_{3}}$", fontsize=14, ha='center')
        ax.text(7, -0.8, r"$\mathbf{E_{3}\text{*}H}$", fontsize=14, ha='center')
        ax.text(7, -1.75, r"$\mathbf{E_{3}\text{*}H_{2}}$", fontsize=14, ha='center')
