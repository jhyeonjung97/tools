...
def main():
    ...
    plt.figure(figsize=(8, 6), dpi=300) # Set the figure size as needed
    ax = sns.heatmap(correlation_matrix, annot=True, fmt=".2f", annot_kws={"size": 6}, cmap='coolwarm', cbar=False)
    ax.set_xticks(np.arange(M.shape[1]) + 0.5)
    ax.set_xticklabels(M.columns, rotation=90, ha='right', fontsize=10)
    ax.set_yticks(np.arange(M.shape[1]) + 0.5)
    ax.set_yticklabels(M.columns, rotation=0, va='center', fontsize=10)
    cbar = plt.colorbar(ax.collections[0], ax=ax, pad=0.02)
