...
def calculate_oer_energies():
    ...
    # Path 1: 1_OOH-Otop2에서 1_O_V → 2_O_OH → 3_O_O → 4_O_OOH
    print("=== Path 1: OOH-Otop2 (O_V → O_OH → O_O → O_OOH) ===")
    try:
        oer_path = root_path / "1_OOH-Otop2"
        ...
        energy_o_ooh = get_energy_from_json(oer_path / "4_O_OOH" / "final_with_calculator.json")
        
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_o_ooh]):
    ...
    # Path 2: 1_OOH-Otop2에서 1_O_V → 2_O_OH → 3_O_O → 5_OH_OO
    print("\n=== Path 2: OOH-Otop2 (O_V → O_OH → O_O → OH_OO) ===")
    try:
        oer_path = root_path / "1_OOH-Otop2"
        energy_o_v = get_energy_from_json(oer_path / "1_O_V" / "final_with_calculator.json")
        energy_o_oh = get_energy_from_json(oer_path / "2_O_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_oh_oo]):
            step1 = (energy_o_oh + dgoh - goh) - (energy_o_v)
            step2 = (energy_o_o + dgo - go) - (energy_o_oh + dgoh - goh)
            step3 = (energy_oh_oo + dgoh + dgoo - goh - goo) - (energy_o_o + 2*dgo - 2*go)
            step4 = 4.92 - step1 - step2 - step3
    ...
    # Path 3: 2_OOH-Mtop2에서 1_V_V → 2_V_OH → 3_V_O → 5_OH_O
    print("\n=== Path 3: OOH-Mtop2 (V_V → V_OH → V_O → OH_O) ===")
    try:
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_v_o, energy_oh_o]):
    ...
    # Path 4: 2_OOH-Mtop2에서 1_V_V → 2_V_OH → 3_V_O → 7_OH-O
    print("\n=== Path 4: OOH-Mtop2 (V_V → V_OH → V_O → OH-O) ===")
    try:
        oer_path = root_path / "2_OOH-Mtop2"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
        energy_v_o = get_energy_from_json(oer_path / "3_V_O" / "final_with_calculator.json")
        energy_oh_o = get_energy_from_json(oer_path / "7_OH-O" / "final_with_calculator.json")
        
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_v_o, energy_oh_o]):
            step1 = (energy_v_oh + dgoh - goh) - (energy_v_v)
            step2 = (energy_v_o + dgo - go) - (energy_v_oh + dgoh - goh)
            step3 = (energy_oh_o + dgooh - gooh) - (energy_v_o + dgo - go)
            step4 = 4.92 - step1 - step2 - step3
    ...
    # Path 5: 2_OOH-Mtop2에서 1_V_V → 2_V_OH → 4_OH_OH → 5_OH_O
    print("\n=== Path 5: OOH-Mtop2 (V_V → V_OH → OH_OH → OH_O) ===")
    try:
        oer_path = root_path / "2_OOH-Mtop2"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
    ...
    # Path 6: 2_OOH-Mtop2에서 1_V_V → 2_V_OH → 4_OH_OH → 7_OH-O
    print("\n=== Path 6: OOH-Mtop2 (V_V → V_OH → OH_OH → OH-O) ===")
    try:
        oer_path = root_path / "2_OOH-Mtop2"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
    ...
    # Path 7: 2_OOH-Mtop2에서 1_V_V → 2_V_OH → 6_OH-OH → 7_OH-O
    print("\n=== Path 7: OOH-Mtop2 (V_V → V_OH → OH-OH → OH-O) ===")
    try:
        oer_path = root_path / "2_OOH-Mtop2"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o]):
            ...
            all_paths['Path7'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
    ...
    # Path 8: 3_OOH-Obrg0에서 1_O_V → 2_O_OH → 3_O_O → 4_O_OOH
    print("\n=== Path 8: OOH-Obrg0 (O_V → O_OH → O_O → O_OOH) ===")
    try:
        oer_path = root_path / "3_OOH-Obrg0"
        energy_o_v = get_energy_from_json(oer_path / "1_O_V" / "final_with_calculator.json")
        energy_o_oh = get_energy_from_json(oer_path / "2_O_OH" / "final_with_calculator.json")
        energy_o_o = get_energy_from_json(oer_path / "3_O_O" / "final_with_calculator.json")
        energy_o_ooh = get_energy_from_json(oer_path / "4_O_OOH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_o_ooh]):
            ...
            all_paths['Path8'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
    ...
    # Path 9: 3_OOH-Obrg0에서 1_O_V → 2_O_OH → 3_O_O → 5_OH_OO
    print("\n=== Path 9: OOH-Obrg0 (O_V → O_OH → O_O → OH_OO) ===")
    try:
        oer_path = root_path / "3_OOH-Obrg0"
        energy_o_v = get_energy_from_json(oer_path / "1_O_V" / "final_with_calculator.json")
        energy_o_oh = get_energy_from_json(oer_path / "2_O_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_oh_oo]):
            ...
            all_paths['Path9'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
    ...
    try:
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_v_o, energy_oh_o]):
            ...
            all_paths['Path10'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
    ...
    try:
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_v_o, energy_oh_o]):
            ...
            all_paths['Path11'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 11: Failed to load some energies")
    except Exception as e:
    ...
    try:
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o]):
            ...
            all_paths['Path12'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 12: Failed to load some energies")
    except Exception as e:
    ...
    try:
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o]):
            ...
            all_paths['Path13'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 13: Failed to load some energies")
    except Exception as e:
    ...
    try:
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o]):
            ...
            all_paths['Path14'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 14: Failed to load some energies")
    except Exception as e:
        print(f"Path 14: Error - {e}")
    
    # Path 15: 5_OOH-Otop3에서 1_O_V → 2_O_OH → 3_O_O → 4_O_OOH
    print("=== Path 15: OOH-Otop3 (O_V → O_OH → O_O → O_OOH) ===")
    try:
        oer_path = root_path / "5_OOH-Otop3"
        energy_o_v = get_energy_from_json(oer_path / "1_O_V" / "final_with_calculator.json")
        energy_o_oh = get_energy_from_json(oer_path / "2_O_OH" / "final_with_calculator.json")
        energy_o_o = get_energy_from_json(oer_path / "3_O_O" / "final_with_calculator.json")
        energy_o_ooh = get_energy_from_json(oer_path / "4_O_OOH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_o_ooh]):
            ...
            all_paths['Path15'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 15: Failed to load some energies")
    except Exception as e:
        print(f"Path 15: Error - {e}")
    
    # Path 16: 5_OOH-Otop3에서 1_O_V → 2_O_OH → 3_O_O → 5_OH_OO
    print("\n=== Path 16: OOH-Otop3 (O_V → O_OH → O_O → OH_OO) ===")
    try:
        oer_path = root_path / "5_OOH-Otop3"
        energy_o_v = get_energy_from_json(oer_path / "1_O_V" / "final_with_calculator.json")
        energy_o_oh = get_energy_from_json(oer_path / "2_O_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_oh_oo]):
            ...
            all_paths['Path16'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 16: Failed to load some energies")
    except Exception as e:
        print(f"Path 16: Error - {e}")

    # Path 17: 6_OOH-Mtop3에서 1_V_V → 2_V_OH → 3_V_O → 5_OH_O
    print("\n=== Path 17: OOH-Mtop3 (V_V → V_OH → V_O → OH_O) ===")
    try:
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_v_o, energy_oh_o]):
            ...
            all_paths['Path17'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 17: Failed to load some energies")
    except Exception as e:
        print(f"Path 17: Error - {e}")
    
    # Path 18: 6_OOH-Mtop3에서 1_V_V → 2_V_OH → 3_V_O → 7_OH-O
    print("\n=== Path 18: OOH-Mtop3 (V_V → V_OH → V_O → OH-O) ===")
    try:
        oer_path = root_path / "6_OOH-Mtop3"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_v_o, energy_oh_o]):
            ...
            all_paths['Path18'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 18: Failed to load some energies")
    except Exception as e:
        print(f"Path 18: Error - {e}")
    
    # Path 19: 6_OOH-Mtop3에서 1_V_V → 2_V_OH → 4_OH_OH → 5_OH_O
    print("\n=== Path 19: OOH-Mtop3 (V_V → V_OH → OH_OH → OH_O) ===")
    try:
        oer_path = root_path / "6_OOH-Mtop3"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o]):
            ...
            all_paths['Path19'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 19: Failed to load some energies")
    except Exception as e:
        print(f"Path 19: Error - {e}")
    
    # Path 20: 6_OOH-Mtop3에서 1_V_V → 2_V_OH → 4_OH_OH → 7_OH-O
    print("\n=== Path 20: OOH-Mtop3 (V_V → V_OH → OH_OH → OH-O) ===")
    try:
        oer_path = root_path / "6_OOH-Mtop3"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o]):
            ...
            all_paths['Path20'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 20: Failed to load some energies")
    except Exception as e:
        print(f"Path 20: Error - {e}")
    
    # Path 21: 6_OOH-Mtop3에서 1_V_V → 2_V_OH → 6_OH-OH → 7_OH-O
    print("\n=== Path 21: OOH-Mtop3 (V_V → V_OH → OH-OH → OH-O) ===")
    try:
        oer_path = root_path / "6_OOH-Mtop3"
        energy_v_v = get_energy_from_json(oer_path / "1_V_V" / "final_with_calculator.json")
        energy_v_oh = get_energy_from_json(oer_path / "2_V_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o]):
            ...
            all_paths['Path21'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 21: Failed to load some energies")
    except Exception as e:
        print(f"Path 21: Error - {e}")
    
    # Path 22: 7_RuO2에서 1_O_V → 2_O_OH → 3_O_O → 4_O_OOH
    print("\n=== Path 22: RuO2 (O_V → O_OH → O_O → O_OOH) ===")
    try:
        ...
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_o_ooh]):
            ...
            all_paths['Path22'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 22: Failed to load some energies")
    except Exception as e:
        print(f"Path 22: Error - {e}")
    
    # Path 23: 7_RuO2에서 1_O_V → 2_O_OH → 3_O_O → 5_OO_OH
    print("\n=== Path 23: RuO2 (O_V → O_OH → O_O → OO_OH) ===")
    try:
        oer_path = root_path / "7_RuO2"
        energy_o_v = get_energy_from_json(oer_path / "1_O_V" / "final_with_calculator.json")
        energy_o_oh = get_energy_from_json(oer_path / "2_O_OH" / "final_with_calculator.json")
        ...
        if all(e is not None for e in [energy_o_v, energy_o_oh, energy_o_o, energy_oo_oh]):
            step1 = (energy_o_oh + dgoh - goh) - (energy_o_v)
            step2 = (energy_o_o + dgo - go) - (energy_o_oh + dgoh - goh)
            step3 = (energy_oo_oh + dgoh + dgoo - goh - goo) - (energy_o_o + 2*dgo - 2*go)
            step4 = 4.92 - step1 - step2 - step3
            ...
            all_paths['Path23'] = [step1, step2, step3, step4]
            print(f"Step1: {step1:.3f} eV, Step2: {step2:.3f} eV, Step3: {step3:.3f} eV, Step4: {step4:.3f} eV")
        else:
            print("Path 23: Failed to load some energies")
    except Exception as e:
        print(f"Path 23: Error - {e}")
...
def plot_oer_energies(all_paths):
    ...
    plt.tight_layout()
    plt.savefig('OER_energies_comparison.png', dpi=300, bbox_inches='tight')
    plt.savefig('OER_energies_comparison.pdf', dpi=300, bbox_inches='tight')
    # plt.show()
    ...
    plt.tight_layout()
    plt.savefig('OER_overpotential_comparison.png', dpi=300, bbox_inches='tight')
    plt.savefig('OER_overpotential_comparison.pdf', dpi=300, bbox_inches='tight')
    # plt.show()
...
def plot_individual_energetics_diagrams(all_paths, oer_data):
    ...
    for i, (path_name, energies) in enumerate(all_paths.items()):
        ...
        filename = f'energetics_diagram_{path_name.lower()}_{path_data["surface"].lower()}.png'
...
def plot_ooh_otop2_pathways(all_paths, oer_data):
    ...
    plt.savefig('OOH_Otop2_pathways_comparison.pdf', dpi=300, bbox_inches='tight')
...
def plot_ooh_otop3_pathways(all_paths, oer_data):
    ...
    plt.savefig('OOH_Otop3_pathways_comparison.pdf', dpi=300, bbox_inches='tight')
...
def plot_all_energetics_diagrams_combined(all_paths, oer_data):
    ...
    plt.tight_layout()
    plt.savefig('all_energetics_diagrams_combined.png', dpi=300, bbox_inches='tight')
    plt.savefig('all_energetics_diagrams_combined.pdf', dpi=300, bbox_inches='tight')
    # plt.show()
    
    print("Saved: all_energetics_diagrams_combined.png")
    
    # 결과를 텍스트 파일로 저장
    with open('OER_energies_comparison.txt', 'w') as f:
