...
def calculate_adsorption_energies():    
    ...
    if len(energies_v_o) == len(energies_o_ohcus) == len(ueff_values):
        ...
        return ueff_values, oh_adsorption_energies_2
    else:
        return None, None

def plot_oh_adsorption_energies_vs_ueff(ueff_values, oh_energies_2):    
    ...
    with open('OH_adsorption_energies_results.txt', 'w') as f:
        f.write("Ueff (eV)\t$\Delta G_{OH}$ (top, O-covered) (eV)\n")
        f.write("-" * 50 + "\n")
        for ueff, oh_energy_2 in zip(ueff_values, oh_energies_2):
            f.write(f"{ueff}\t\t{oh_energy_2:.6f}\n")

if __name__ == "__main__":
    ueff_values, oh_energies_2 = calculate_adsorption_energies()
    
    if ueff_values is not None and oh_energies_2 is not None:
        plot_oh_adsorption_energies_vs_ueff(ueff_values, oh_energies_2)