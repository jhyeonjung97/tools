#!/bin/bash
# Sum ICOHP (spin1+spin2 total) for dopant(5d)-O(2s+2p) bonds in the current
# directory's ICOHPLIST.lobster. Usage: parse_dopant5d_osp.sh <ElementSymbol>
...
el="$1"
if [ -z "$el" ] || [ ! -e ICOHPLIST.lobster ]; then
    echo ""
    exit 0
fi
...
awk -v el="$el" -v two_col="$two_col" '
(($2 ~ "^"el"[0-9]+_5d") && ($3 ~ /^O[0-9]+_2[sp]/)) ||
(($3 ~ "^"el"[0-9]+_5d") && ($2 ~ /^O[0-9]+_2[sp]/)) {
    if (two_col) {
