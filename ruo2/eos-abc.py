...
df = df.loc[df.groupby(['b', 'c'])['energy'].idxmin()].reset_index(drop=True)
print(df)
# plt.scatter(df['a'], df['c'], c=df['energy'], edgecolors='black', linewidths=0.5, cmap='RdBu', zorder=10)
plt.scatter(df['b'], df['c'], c=df['energy'], edgecolors='black', linewidths=0.5, cmap='RdBu', zorder=10)
# plt.tricontourf(df['a'], df['c'], df['energy'], levels=30, cmap='RdBu', zorder=0)
plt.tricontourf(df['b'], df['c'], df['energy'], levels=30, cmap='RdBu', zorder=0)
...
plt.xlabel('Lattice constant b (Å)')
plt.ylabel('Lattice constant c (Å)')
plt.tight_layout()
plt.savefig('eos-abc.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()