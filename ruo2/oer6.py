...
def calculate_oer_energies():
    ...
    # Path 1: 6단계 OER 경로 (V_V → V_OH → OH_OH → OH_O → O_O → O-O)
    print("=== Path 1: 6-step OER (V_V → V_OH → OH_OH → OH_O → O_O → O-O) ===")
    try:
        ...
        energy_v_oo = get_energy_from_json(oer_path / "9_O-O" / "final_with_calculator.json")
        
        if all(e is not None for e in [energy_v_v, energy_v_oh, energy_oh_oh, energy_oh_o, energy_o_o, energy_v_oo]):
            ...
            step5 = (energy_v_oo + dgoo) - (energy_o_o + 2*dgo)
    ...
    # Path 2: 6단계 OER 경로 (V_V → V_OH → OH_OH → OH_O → O_O → O-O)
    print("=== Path 2: 6-step OER (V_V → V_OH → OH_OH → OH_O → O_O → O-O) ===")
    try:
...
def plot_oer_energies(all_paths):
    ...
    for i, (path_name, energies) in enumerate(all_paths.items()):
        # 길이 불일치를 방지하기 위해 필요한 길이로 자름
        energies_to_plot = energies[:len(steps)]
        bars = ax1.bar(x, energies_to_plot, width, label=path_name, color=colors[i % len(colors)], alpha=0.8, edgecolor='black', linewidth=1)
        
        # 각 막대 위에 값 표시
        for j, (bar, energy) in enumerate(zip(bars, energies_to_plot)):
...
def plot_all_energetics_diagrams_combined(all_paths, oer_data):
    ...
    ax.set_title('OER Energetics Diagrams - 6-step Path', fontsize=16, fontweight='bold')
    ...
    # 결과를 텍스트 파일로 저장
    with open('OER_energies_comparison_6step.txt', 'w') as f:
        f.write("OER Energetics Comparison - 6-step Path\n")
        f.write("=" * 60 + "\n\n")
