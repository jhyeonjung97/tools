...
def main():
    ...
    for row in rows:
        for i, element in enumerate(elements[row]):
            atomic_number = mendeleev.element(element).atomic_number
            atoms_mruo2 = os.path.join(base_path, f'4_M-RuO2_all/{atomic_number}_{element}/final_with_calculator.json')
            atoms_mxoy = os.path.join(base_path, f'6_MxOy_all/{atomic_number}_{element}/final_with_calculator.json')
            if element == 'Ni':
                print(atoms_mruo2, atoms_mxoy)
            if os.path.exists(atoms_mruo2) and os.path.exists(atoms_mxoy):
                ...
                plt.xlim(-0.1, 1.1)
                plt.ylim(-0.3, 0.3)
                plt.savefig(f'RuO2_{formula}_convex_hull.png', dpi=300, bbox_inches='tight')
                plt.tight_layout()
                if show:
                    plt.show()
                plt.close()
