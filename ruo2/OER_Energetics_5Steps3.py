...
def calculate_oer_energies():
    ...
    try:
        oer_path = root_path / "5_ReRuO2_alloy" / "1_OER"
        energy_o_v = get_energy_from_json(oer_path / "1_O_V" / "final_with_calculator.traj") + gibbs_correction_o_v
        energy_o_oh = get_energy_from_json(oer_path / "2_O_OH" / "final_with_calculator.traj") + gibbs_correction_o_oh
        energy_o_o = get_energy_from_json(oer_path / "3_O_O" / "final_with_calculator.traj") + gibbs_correction_o_o
        energy_oh_oo = get_energy_from_json(oer_path / "5_OO_OH" / "final_with_calculator.traj") + gibbs_correction_oh_oo
...
def print_oer_table(oer_data):
    ...
    for i, data in enumerate(oer_data, 1):
        ...
        row = f"{data['surface']:<12} {data['int1']:<8} {data['int2']:<8} {data['int3']:<8} {data['int4']:<8} {data['int5']:<8} " \
              f"{data['step1']:<10.3f} {data['step1']+data['step2']:<10.3f} {data['step1']+data['step2']+data['step3']:<10.3f} {data['step1']+data['step2']+data['step3']+data['step4']:<10.3f} {data['step1']+data['step2']+data['step3']+data['step4']+data['step5']:<10.3f} " \
...
def plot_all_pathways_combined_RuO2(all_paths, oer_data):
    ...
    # 필터링할 surface 목록
    target_surfaces = ['RuO2(d)', 'ReRuO2', 'RuO2_1%', 'RuO2_2%', 'ReRuO2-alloy']
    names = ['RuO$_2$', 'Re(brg)-RuO$_2$', 'RuO$_2$(+1%)', 'RuO$_2$(+2%)', 'Re-RuO$_2$ alloy'] #'Re(brg)-RuO$_2$(+1%)', 'Re(brg)-RuO$_2$(+2%)']
    ...
    plt.legend(fontsize=12, loc='upper left', bbox_to_anchor=(1.0, 1.0))
    # plt.tight_layout()
    # plt.show()
    plt.savefig('all_pathways_combined_5steps.png', dpi=300, bbox_inches='tight')
    plt.savefig('all_pathways_combined_5steps.pdf', dpi=300, bbox_inches='tight')
