...
def main():
    ...
    # 파일명 정의
    log_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}.log')
    tsv_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}.tsv')
    png_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}.png')
    importance_png_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}_importance.png')
    json_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}.json')

    # coordination 분석 결과 파일 경로
    coord_csv_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}_coord.csv')
    coord_log_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}_coord.log')
    coord_png_path = os.path.join(root, f'{args.Y}_pred_regulation_{args.model}{threshold_str}_{row_str}_{output_suffix}_coord.png')
    ...
    if args.model == 'gpr':
        # GPR model setup - 정규화 강화
        kernel = ConstantKernel(1.0, constant_value_bounds=(1e-3, 1e5)) * RationalQuadratic(
            length_scale=1.0,
            alpha=1.0,
            length_scale_bounds=(1e-3, 1e6),
            alpha_bounds=(1e-3, 1e6)
        ) + WhiteKernel(noise_level=0.2, noise_level_bounds=(1e-3, 1e5))  # 노이즈 증가
        ...
        # RF model setup - 정규화 강화
        model = RandomForestRegressor(
            n_estimators=100,
            max_depth=6,
            min_samples_split=8,
            min_samples_leaf=4,
            max_features=0.7,  # 특성 수 제한
            random_state=args.random_state,
            ccp_alpha=0.01  # 가지치기 강화
        )
        ...
        model = lgb.LGBMRegressor(
            ...
            reg_lambda=2.0,  # L2 정규화 강화
            min_child_samples=5,  # 증가
            min_child_weight=0.01  # 추가
        )
    ...
    if args.save:
        result_filename = f'{args.Y}_pred_regulation_{args.model}{threshold_str}_all_result.log'
