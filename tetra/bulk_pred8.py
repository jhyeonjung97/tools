...
from sklearn.gaussian_process.kernels import RationalQuadratic, WhiteKernel, ConstantKernel, RBF
...
def get_scaler(scaler_type):
    """Get the appropriate scaler based on the specified type"""
...
def feature_selection(X, y, method='rfe', model_type='gpr', n_features=None):
    ...
    if method == 'rfe':
        ...
        selector = RFECV(base_model, cv=5)
...
def main():
    ...
    parser.add_argument('--pca', action='store_true', help='Apply Principal Component Analysis')
    ...
    try:
        ...
        if args.pca:
            ...
            X_train_scaled = pd.DataFrame(pca.fit_transform(X_train_scaled), index=X_train_scaled.index)
            X_test_scaled = pd.DataFrame(pca.transform(X_test_scaled), index=X_test_scaled.index)
        ...
        if args.model == 'gpr':
            ...
            model = Ridge(random_state=args.random_state)
            search_space = {
        ...
        if args.model == 'gpr':
            ...
            y_pred, std = model.predict(X_scaled, return_std=True)
        else:
            ...
            std = get_prediction_std(model, X_scaled, model_type=args.model)
        ...
        with open(log_path, 'w') as f:
            ...
            f.write(f"MAE: {mean_absolute_error(y_train, model.predict(X_train_scaled)):.4f}\n")
            f.write(f"MSE: {mean_squared_error(y_train, model.predict(X_train_scaled)):.4f}\n")
            f.write(f"R2: {r2_score(y_train, model.predict(X_train_scaled)):.4f}\n\n")
            f.write("Test Metrics:\n")
            f.write(f"MAE: {mean_absolute_error(y_test, model.predict(X_test_scaled)):.4f}\n")
            f.write(f"MSE: {mean_squared_error(y_test, model.predict(X_test_scaled)):.4f}\n")
            f.write(f"R2: {r2_score(y_test, model.predict(X_test_scaled)):.4f}\n\n")

        # Calculate feature importance
        importance = feature_importance(X_train_scaled, y_train, model, X_train_scaled.columns, args.model)
        plot_feature_importance(X_train_scaled, X_test_scaled, y_train, y_test, X_train_scaled.columns, 
