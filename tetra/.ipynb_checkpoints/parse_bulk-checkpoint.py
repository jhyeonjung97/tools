...
float_cols = ['energy', 'form', 'coh', 'volume', 'cell', 'chg', 'mag', 'l_bond', '-ICOHP', 'ICOBI', '-ICOOP', '-ICOHPn', 'ICOBIc', '-ICOOPc', 'madelung', 'grosspop']
        ...
        for row in metals.keys():
            for m, metal in enumerate(metals[row]):
                ...
                if os.path.exists(icohp_path) and os.path.getsize(icohp_path) != 0:
                    ...
                    df.loc[item, ['l_bond', 'n_bond', '-ICOHPn', 'ICOBIc', '-ICOOPc', 'madelung', 'grosspop']] = bond, nbond, icohp, icobi, icoop, madelung/MN, grosspop
                    df.loc[item, ['-ICOHP', 'ICOBI', '-ICOOP']] = icohp*nbond, icobi*nbond, icoop*nbond
