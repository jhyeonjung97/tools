...
def main():
    ...
    test_mses = [h['test_mse'] for h in history]
    feature_names_order = [h['features'][-1] for h in history]
    import matplotlib.ticker as ticker
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(range(1, len(test_maes)+1), test_maes, marker='o', label='Test MAE')
    ax.plot(range(1, len(test_mses)+1), test_mses, marker='s', label='Test MSE')
    ax.set_xlabel('Number of Features')
    ax.set_ylabel('Error')
    ax.set_xticks(range(1, len(test_maes)+1))
    ax.xaxis.set_major_locator(ticker.MultipleLocator(1))
    # 위쪽에 feature 이름 표시
    ax2 = ax.twiny()
    ax2.set_xlim(ax.get_xlim())
    ax2.set_xticks(range(1, len(feature_names_order)+1))
    ax2.set_xticklabels(feature_names_order, rotation=90, fontsize=9)
    ax2.set_xlabel('Feature Added (in order)', labelpad=10)
    ax.legend()
    fig.tight_layout()
