...
# 선 그리기
yy1 = -np.maximum(-xx+0.4, 0.7*xx+1.3)
yy2 = -np.maximum(-xx+0.4, 0.8*xx+1.5)
yy3 = -np.maximum(-xx+0.4, 0.9*xx+1.7)
...
plt.plot(xx_left, -(-xx_left+0.2), '--', color='black', lw=0.67, dashes=(3, 1), zorder=3)

# 선들 사이의 공간을 색칠
plt.fill_between(xx, yy1, yy3, alpha=0.2, color='silver', zorder=1)

plt.plot(xx, -np.maximum(-0.5*xx, 0.5*xx), '--', color='silver', lw=0.67, dashes=(3, 1), zorder=2)
...
for i in range(len(systems)):
    x = systems[i][1]
    plt.scatter(x, -max(-0.5*x, 0.5*x), color='silver', marker=systems[i][3], linewidth=0.67, zorder=4, s=12)
    plt.scatter(x, -max(-x+0.2, 0.7*x+1.3), facecolor=systems[i][2], edgecolor='black', marker=systems[i][3], linewidth=0.67, zorder=4)
    if -x+0.2 < 0.9*x+1.5:
        plt.scatter(x, -max(-x+0.2, 0.8*x+1.5), facecolor='white', edgecolor='silver', marker=systems[i][3], linewidth=0.67, zorder=4, s=12)
        plt.scatter(x, -max(-x+0.2, 0.9*x+1.7), facecolor='white', edgecolor='silver', marker=systems[i][3], linewidth=0.67, zorder=4, s=12)
    # if systems[i][0] == 'CuN4':
...
plt.text(-1.4, 0.05, r'E°$_{NO_{3}^{-}/NH_{3}}$ = 0.70 V vs RHE', color='black', fontsize=8)
# plt.xticks([])
# plt.yticks([])
...
plt.xlim(x1, x2)
plt.ylim(y1, y2)
plt.xlabel(r'$\Delta G_{\sf NO}$ (eV)')
