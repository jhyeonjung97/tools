...
def main():
    ...
    # Print all entry energies at pH=0, U=0 V vs SHE
    print(f"\n=== Entry Energies at pH=0, U=0 V vs SHE ===")
    energies_at_origin = [
        (k, surfs[k]['name'], dg(surfs[k], pH=0, U=0, ref_surf=surfs[ref_surf_idx]))
        for k in range(nsurfs)
    ]
    for _, name, energy in sorted(energies_at_origin, key=lambda x: x[2]):
