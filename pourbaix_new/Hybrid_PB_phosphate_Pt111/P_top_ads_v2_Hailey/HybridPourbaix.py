...
def dg(surf, pH, U, ref_surf):
    ...
    surface_term = (
        (surf['A'] * U ** 2 + surf['B'] * U + surf['dG'])
        - (ref_surf['A'] * U ** 2 + ref_surf['B'] * U + ref_surf['dG'])
    )
...
def build_surfs(json_files, file_labels, file_gibbs_corrections, file_gc_params, args):
    ...
    for json_file in json_files:
        ...
        row = {'dG': atoms.get_potential_energy(), 'e': 0, 'conc': 1}
...
def find_new_ref_surface(surfs, ref_surf, unique_elements):
    ...
    def pick_lowest(candidates):
        idx, surf = min(candidates, key=lambda item: float(item[1]['dG']))
        return surf, idx
...
def surface_formation_corrections(surfs, ref_surf, file_oh_counts, args, unique_elements, ref_energies):
    ...
    for row in surfs:
        ...
        row['dG'] = row['dG'] + gibbs_correction + formation_correction

    ref_surf_energy = ref_surf['dG']
    for row in surfs:
        row['dG'] = row['dG'] - ref_surf_energy
...
def surf_df_columns(remaining_elements, is_gc):
    ...
    cols = ['dG', 'e'] + remaining_elements + ['conc', 'name']
    if is_gc:
        cols += ['A', 'B']
    return cols
...
def main(args, png_name, suffix):
    ...
    if args.hybrid:
        species_cols = ['dG', 'e'] + remaining_elements + ['conc', 'name']
