...
# 그래프에 사용한 데이터 저장
graph_tsv_output = os.path.join(base_dir, 'bader_charges.tsv')
graph_csv_output = os.path.join(base_dir, 'bader_charges.csv')
...
for ads in selected_adsorbates:
    ...
    plot_output = os.path.join(base_dir, f'bader_charges_{ads_name}.png')
...
if len(slab_subset) > 0:
    ...
    plot_output = os.path.join(base_dir, 'bader_charges_slab.png')
    plt.savefig(plot_output, bbox_inches='tight')
    plt.close()
    print(f"Plot saved to {plot_output}") 