from ase.io import read
import os
import pandas as pd
# 기체 분자의 에너지와 보정값
E_H2 = -6.77108058  # H2의 에너지 (eV)
E_H2O = -14.22266334  # H2O의 에너지 (eV)

# ZPE, 열용량, 엔트로피 보정
ZPE_H2O = 0.558
CV_H2O = 0.103
TS_H2O = 0.675

ZPE_H2 = 0.268
CV_H2 = 0.0905
TS_H2 = 0.408

# 기체 분자의 깁스 자유에너지
G_H2O = E_H2O + ZPE_H2O - TS_H2O + CV_H2O
G_H2 = E_H2 + ZPE_H2 - TS_H2 + CV_H2

root = '/Users/hailey/Desktop/4_IrFe3/'
surfaces = ['Ir', 'IrFe']
df = pd.DataFrame(columns=surfaces, index=range(1, 9))

for s in range(len(surfaces)):
    clean_path = os.path.join(root, 'slab', f'{s*6}_{surfaces[s]}', 'final.json')
    clean_energy = read(clean_path).get_potential_energy()

    for i in range(1, 9):
        atoms_path = os.path.join(root, 'oxide', f'{s+7}_{surfaces[s]}Ox', f'{i}_', 'final.json')
        atoms = read(atoms_path)
        energy = atoms.get_potential_energy()
        potential = ((energy - clean_energy)/16 - (G_H2O - G_H2)) / 2
        df.loc[i, surfaces[s]] = potential

print(df)
df.to_csv('/Users/hailey/Desktop/4_IrFe3/figures/oxide.csv')