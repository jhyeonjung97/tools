...
H_EXTRA_STRUCTURE_ANNOTATIONS = [
    {"stem": "a-h(0.25)mno2", "xytext": (-10, 0), "va": "center", "ha": "right"},
    {"stem": "r-h(0.25)mno2", "xytext": (0, -2), "va": "top", "ha": "right"},
    {"stem": "e-h(0.33)mno2", "xytext": (-6, 0), "va": "center", "ha": "right"},
    # {"stem": "g-h(0.33)mno2", "xytext": (-6, 0), "va": "top", "ha": "right"},
    # {"stem": "spinel-h(0.5)mno2", "xytext": (-6, 0), "va": "top", "ha": "right"},
...
def display_structure_name(name: str) -> str:
    ...
    if name.lower() == "h-hmn2o4":
        return "Sp-HMn2O4 (Hetaerolite)"
...
def reduced_formula_key(counts):
    """Return reduced H-Mn-O composition key, e.g. H1Mn2O4."""
    n_h = counts.get("H", 0)
...
def find_mno2_reference_energy(sherry_dir: Path):
    ...
    for path in sorted(sherry_dir.glob("*.json")):
        ...
        # Exact MnO2 stoichiometry only (no H/H/etc.)
...
def build_points(sherry_dir: Path, mn02_ref_energy: float, include_no_dash: bool = False):
    """Build (x_H, formation_energy) points from *H*.json files."""
    ...
    for path in sorted(sherry_dir.glob("*H*.json")):
        ...
        total_metal = n_h + n_mn
        x_h = n_h / total_metal
        ratio = n_h / (n_mn-n_h)
        total_energy = atoms.get_potential_energy()

        # Reference: n_Mn * E(MnO2_min) + n_H * E(H_metal_ref)
        formation_energy = (
            total_energy - n_mn * mn02_ref_energy - n_h * H_REFERENCE_ENERGY
        ) / total_metal
    ...
    for p in points:
        print(p)
    if not points:
        raise RuntimeError("No valid H-containing Mn structures found in sherry directory.")
    return points
...
def main():
    parser = argparse.ArgumentParser(
        description="Build H-composition convex hull from mno2 sherry JSON files."
    )
    parser.add_argument(
        "--sherry-dir",
        type=str,
    ...
    endpoint_h = {"name": "H_ref", "x": 1.0, "y": 0.0}
    hull_points = lower_hull(points + [endpoint_mno2, endpoint_h])

    print(f"Using sherry directory: {sherry_dir}")
    print(f"H reference energy: {H_REFERENCE_ENERGY:.10f} eV/H")
    print(f"MnO2 reference (most stable): {mn02_ref_name}")
    print(f"MnO2 reference energy: {mn02_ref_energy:.10f} eV per MnO2")
    print(f"Loaded H-containing structures: {len(points)}")
    ...
    for phase in phases:
        ...
        phase_hull_candidates.append({"name": "H_ref", "x": 1.0, "y": 0.0})
        ...
        plt.scatter(
            ...
            edgecolors="black",
            linewidths=0.3,
            label=f"{display_phase_name(phase)}" if phase != "H" else "_nolegend_",
            zorder=z_scatter,
        )
    ...
    plt.ylim(-0.4, 0.3)
    plt.xlabel(r"Mn$^{3+}$/Mn$^{4+}$", fontsize=12)
    plt.ylabel(r"Formation energy ($\Delta E$, eV)", fontsize=12)
    plt.legend(loc="upper right", handletextpad=0.1)
    plt.tight_layout()
    output_path = sherry_dir / f"convex-hull-mno2-h-{args.palette}-small-legend.pdf"
